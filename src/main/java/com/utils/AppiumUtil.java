...
public class AppiumUtil {
    ...
    public static void click(AppiumDriver<WebElement> driver, By by) {
        try {
            ...
            LogZ.info(by.toString() + " = 控件点击成功" );
        } catch (NoSuchElementException e){
            LogZ.error(by.toString() + " = 控件不存在或无法获取到" );
    ...
    public static Boolean clickB(AppiumDriver<WebElement> driver,By by) {
        try {
            ...
            LogZ.debug(by.toString() + " = 控件不存在或无法获取到" );
            return false;
        /* driver.findElement(by.id(by));
        driver.findElement(By.name(by));
        driver.findElement(By.className(by));
    ...
    public static void click(AppiumDriver<WebElement> driver,By by, int index) {
        ...
        try {
            ...
            targetEle.click();
            LogZ.info(by.toString() + " 正在点击列表控件，当前点击序号为 ：" + index + " 总共：" + size);
        } catch (IndexOutOfBoundsException ioobe){
            LogZ.error( by.toString() + " 当前点击序号为：" + index + " = 控件不存在或无法获取到。"  + " 总共：" + size);
                /* driver.findElement(by.id(by));
                driver.findElement(By.name(by));
                driver.findElement(By.className(by));
                driver.findElement(By.xpath(by));*/
    ...
    public static Boolean clickB(AppiumDriver<WebElement> driver,By by, int index) {
        ...
        try {
            ...
            LogZ.debug( by.toString() + " 当前点击序号为：" + index + " = 控件不存在或无法获取到。"  + " 总共：" + size);
            return false;
/*                 driver.findElement(by.id(by));
                driver.findElement(By.name(by));
                driver.findElement(By.className(by));
    ...
    public static boolean isExist(AppiumDriver<WebElement> driver,By by){
        try{
            before(driver);
            driver.findElement(by);
//            LogZ.info(by.toString() + " = 控件存在且已获取到" );
            return true;
        }catch(Exception e){
            LogZ.error(by.toString() + " = 控件不存在或无法获取到" );
            return false;
    ...
    public static boolean isExistIndex(AppiumDriver<WebElement> driver,By by, int index){
        ...
        try {
            ...
            targetEle = byList.get(index);
            if(targetEle != null) {
                LogZ.info(targetEle.toString() + " 要查找的控件存在，当前点击序号为 ：" + index + " 总共：" + size);
            } else {
                LogZ.info(targetEle.toString() + " 要查找的控件不存在，当前点击序号为 ：" + index + " 总共：" + size);
            }
            return true;
        } catch (IndexOutOfBoundsException ioobe){
            LogZ.error(by.toString() + " 当前点击序号为：" + index + " = 控件不存在或无法获取到。" + " 总共：" + size);
    ...
    public static void typeWriting(int device){
        switch (device){
            case 1 :
                TestUtils.excuteAdbShell(Constant.MEIZU_INPUT);
                break;
            case 2 :
                TestUtils.excuteAdbShell(Constant.HUAWEI_INPUT);
        }
    }
}
